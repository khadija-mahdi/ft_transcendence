...
async function fetchAllPlayer(q) {
	...
	if (!q || q === '') {
		pendingurl = "https://localhost:4433/api/v1/users/appending-requests/";
		recommendedUrl = "https://localhost:4433/api/v1/users/recommended-users/";
	}
	else
		recommendedUrl = `https://localhost:4433/api/v1/users/search-user/?none_friend_only=true&search_query=${q}`
...
async function SendFriendRequest(userId) {
	document.querySelector('.invite-button').addEventListener('click', async function() {
		try{
			const svgContainer = document.querySelector('.invite-content svg');
			...
			if (svgContainer && inviteText) {
				...
				inviteText.textContent = 'Friend Request Sent';
			}
    ...
    function renderRecommendedList(Recommended) {
        ...
        if (!Recommended.length) {
            ...
            Recommended.forEach(friend => {
				...
				console.log("re id:", Recommended.id)
				SendFriendRequest(Recommended.id)
