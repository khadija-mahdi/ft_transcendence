...
class GoogleUserSerializer(serializers.ModelSerializer):
    ...
    def save(self, *args, **kwargs):
        ...
        self.validated_data['first_name'] = self.initial_data.get(
            'given_name', '')
        self.validated_data['last_name'] = self.initial_data.get(
            'family_name', '')
...
class IntraUserSerializer(serializers.ModelSerializer):
    ...
    def save(self, *args, **kwargs):
        ...
        self.validated_data['image_url'] = self.initial_data.get(
            'image', '').get('versions').get('medium')
        self.validated_data['first_name'] = self.initial_data.get(
            'displayname', '').split(' ')[0]
        self.validated_data['last_name'] = self.initial_data.get(
            'displayname', '').split(' ')[1]
